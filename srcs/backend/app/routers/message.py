...
async def llm(frames, gt, websocket, model="gpt-4o", aggregated_data=None):  # ✅ Ajout d'async
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    if aggregated_data:
        USER_INSTRUCTION = """Provide a detail summary of the session of the patient based on this feedbacks:
        Feedbacks: {aggregated_data}
        """
        messages = [
